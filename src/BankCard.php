...
namespace YL20181120\BankCardInfo;
...
class BankCard
{
    ...
    public static function info($cardNum, $type = 'bank')
    {
        if ($type != 'bank') {
            return self::other($cardNum, $type);
        }
    ...
    private static function other($cardNum, $type)
    {
        $type     = strtoupper($type);
        $bankInfo = array(
            'validated'    => true,              // 是否验证通过
            'bank'         => $type,                        // 银行代码
            'bankName'     => isset(self::$bankInfo[$type]) ? self::$bankInfo[$type] : '',   // 银行名称
            'bankImg'      => '',
